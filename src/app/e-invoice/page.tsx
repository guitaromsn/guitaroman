...
import { metalScrapItems, currencies, paymentTerms } from '@/data/metalScrapItems';
import { Customer, FormItem, MetalScrapItem } from '@/types';
import { 
  DocumentTextIcon, 
  PrinterIcon, 
  PaperAirplaneIcon,
...
export default function EInvoicePage() {
  ...
  const [selectedItems, setSelectedItems] = useState<MetalScrapItem[]>([]);
  ...
  useEffect(() => {
    setFormData(prev => ({
      ...prev,
      documentNumber: `EINV-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`
    }));
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    alert('E-Invoice created successfully and will be submitted to ZATCA!');
    setIsLoading(false);
  };

  const handleSave = () => {
    alert('E-Invoice saved as draft');
  };
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground">E-Invoice</h1>
              <p className="text-foreground-secondary">ZATCA Compliant Electronic Invoice</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Select
                ...
                options={paymentTerms.map(term => ({ value: term.value, label: term.labelEn }))}
              />
              <Select
                label="Currency"
          ...
          {/* Totals */}
          {formData.items.length > 0 && (
            <div className="card p-6 bg-background-secondary border-primary/20">
